...
class MultiInputLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, output_size):
        ...
        # self.CMD_LSTM = nn.LSTM(8*11, 32, 5, batch_first=True,dropout=0.2)
        # self.bn1 = nn.BatchNorm1d(32)

        # self.Real_LSTM = nn.LSTM(8*11, 32, 5, batch_first=True,dropout=0.2)
        ...
        self.max_pooling1 = nn.MaxPool1d(3, stride=2)
        self.max_pooling2 = nn.MaxPool1d(3, stride=2)
    ...
    def forward(self, x1,x2):
        x1 = x1.view(-1,1,24)
        real_out, _ = self.Real_LSTM(x1)
        real_out = self.bn1(real_out[:, -1, :]) 

        x2 = x2.view(-1,1,24)
        cmd_out, _ = self.CMD_LSTM(x2) # LSTM层
        cmd_out = self.bn2(cmd_out[:, -1, :]) 
        
        stack_data = torch.stack((real_out,cmd_out),dim=0).permute(1, 0, 2)
        stack_data = stack_data.view(-1,1,2,64)
        stack_data = self.relu1(self.conv_layer3(stack_data))
        stack_data = stack_data.view(-1,16,64)

        out = self.max_pooling1(stack_data)    
        out = out.view(-1,1,496)

        # stack_data = (real_out + cmd_out)/2
        # out = stack_data.view(-1, 1, 32)
        out,_ = self.concat_LSTM(out)
...
def train(device):
    ...
    # test_data, test_data_2 ,test_labels = loadData(r"data\test.txt",device,0.2)
    ...
    for epoch in range(num_epochs):
        for i, (images, train_labels) in enumerate(dataloader):
            ...
            if iter % 500 == 0:
                ...
                # print(f'Iteration: {epoch}\t Loss: {avg_.item():.4f}')
                # Calculate Loss
...
def test(device):
    ...
    train_, test_ = loadData(r"data\train.txt",device,0.8)
    test_data, test_data_2 ,test_labels = test_
